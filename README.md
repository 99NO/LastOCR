...
- 현재 구현은 `"."` Symbol 위치 주변의 원본 이미지 픽셀을 crop한 뒤 어두운 connected component를 찾아 실제 마침표 잉크 중심을 우선 사용합니다. 실패하면 ML Kit Symbol 박스 중심으로 fallback합니다.
- 로그의 `이미지점`은 픽셀 기반 점 검출 성공, `MLKit박스`는 fallback을 뜻합니다.
